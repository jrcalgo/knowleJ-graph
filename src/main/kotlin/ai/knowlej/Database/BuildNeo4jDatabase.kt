...
open class BuildNeo4jDatabase {
    ...
    fun createNewDatabase(newDatabaseName: String?): Boolean {
        ...
        if (!databaseExistence) {
            ...
            try {
                openSession('w').use { _ ->
                    val parameter = mapOf("newDatabaseName" to newDatabaseName)
                    val result = session!!.run("CREATE DATABASE \$newDatabaseName", parameter)
                    if (result.hasNext())
        ...
        fun checkForDomain(domainNode: DomainGroupNode, continueReadSession: Boolean = false): Boolean {
            ...
            try {
                ...
                val parameters = mapOf(
                    "domainName" to domainNode.domainName,
                    "labels" to domainNode.domainLabels.joinToString(":"),
                    "domainProperties" to domainNode.domainProperties
                )
                val cypher =
                    "MATCH (d:Domain:\$labels {name: \$domainName, properties: \$domainProperties}) WHERE d.name = " +
                            "\$domainName RETURN d"
        ...
        fun createDomainNode(domainNode: DomainGroupNode): org.neo4j.driver.types.Node? {
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val cypher =
                        "CREATE (d:Domain:\$labels {name: \$domainName, properties: \$domainProperties}) RETURN d"
                    val result = session!!.run(cypher, parameters)
                    if (result.hasNext()) {
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val domain1Parameters = mapOf(
                        ...
                        "domainProperties1" to domainNode1.domainProperties
                    )
                    val domain2Parameters = mapOf(
                        "domainName2" to domainNode2.domainName,
    ...
    open inner class BuildSubdomainNodes : BuildDomainNodes() {
        fun checkForSubdomain(
            domainNode: DomainGroupNode,
            subdomainNode: SubdomainGroupNode,
            continueReadSession: Boolean = false
        ): Boolean {
            //  check if subdomain and parent domain already exists
            var subdomainExistence: Boolean = false
            try {
                if (checkForDomain(domainNode, true)) {
                    val domainParameters = mapOf(
                        "domainName" to domainNode.domainName,
                        "domainLabels" to if (domainNode.domainLabels.isNotEmpty())
                            domainNode.domainLabels.joinToString(":") else "",
                        "domainProperties" to if (domainNode.domainProperties.isNotEmpty())
                            domainNode.domainProperties else ""
                    )
                    val subdomainParameters = mapOf(
                        "subdomainName" to subdomainNode.subdomainName,
                        "subdomainLabels" to subdomainNode.subdomainLabels.joinToString(":"),
                        "subdomainProperties" to subdomainNode.subdomainProperties
                    )
                    val cypher =
                        "MATCH (d:Domain:\$domainLabels {name: \$domainName, properties: \$domainProperties})" +
                                "-[:DOMAIN_OF]-(s:Subdomain:\$subdomainLabels {name: \$subdomainName, properties: " +
                                "\$subdomainProperties}) RETURN s"
        ...
        fun createSubdomainNode(
            ...
            try {
                openSession('w').use { _ ->
                    val domainParameters = mapOf(
                        "domainName" to domainNode.domainName,
                        "domainLabels" to domainNode.domainLabels.joinToString(":"),
                        "domainProperties" to domainNode.domainProperties
                    )
                    val subdomainParameters = mapOf(
                        "subdomainName" to subdomainNode.subdomainName,
                    ...
                    val cypher =
                        "MERGE (d:Domain {name})-[:DOMAIN_OF]-(s:Subdomain: \$subdomainLabels {name: " +
                                "\$subdomainName, properties: \$subdomainProperties} RETURN s"
                    val result = session!!.run(cypher, parameters)
                    if (result.hasNext())
        ...
        fun createSubdomainRelationship(
            domainNode1: DomainGroupNode,
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val fromDomainParameters = mapOf(
                        ...
                        "fromDomainProperties" to domainNode1.domainProperties
                    )
                    val fromSubdomainParameters = mapOf(
                        "fromSubdomainName" to subdomainNode1.subdomainName,
                        "fromSubdomainLabels" to subdomainNode1.subdomainLabels.joinToString(":"),
                        "fromSubdomainProperties" to subdomainNode1.subdomainProperties
                    )
                    val toDomainParameters = mapOf(
                        "toDomainName" to domainNode2.domainName,
                        "toDomainLabels" to domainNode2.domainLabels.joinToString(":"),
                        "toDomainProperties" to domainNode2.domainProperties
                    )
                    val toSubdomainParameters = mapOf(
                        "toSubdomainName" to subdomainNode2.subdomainName,
                        "toSubdomainLabels" to subdomainNode2.subdomainLabels.joinToString(":"),
    ...
    open inner class BuildKnowledgeBaseNodes : BuildSubdomainNodes() {
        fun createAbstractKB(
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val abstractKBParameters = mapOf(
                        "abstractKBName" to abstractKBNode.abstractKBName,
                        "abstractKBLabels" to abstractKBNode.abstractKBLabels.joinToString(":"),
                        "abstractKBProperties" to abstractKBNode.abstractKBProperties
                    )
                    val parameters = domainParameters + subdomainParameters + abstractKBParameters
                    var cypher =
                        "MERGE (d:Domain:\$domainLabels {name: \$domainName, properties: \$domainProperties})" +
                                "-[:DOMAIN_OF]->(s:Subdomain:\$subdomainLabels {name: \$subdomainName, properties: " +
                                "\$subdomainProperties})-[:SUBDOMAIN_OF]->(a:AbstractKB:\$abstractKBLabels {name: " +
                                "\$abstractKBName, properties: \$abstractKBProperties}) " +
                                "ON CREATE RETURN a ON MATCH RETURN null"
                    ...
                    if (result.hasNext()) {
                        ...
                        for (node in abstractKBNode.abstractKnowledgeNodes) {
                            val childParameters = mapOf(
                                "sentence" to node.concept,
                                "sentenceLabels" to node.conceptLabels.joinToString(":"),
                                "sentenceProperties" to node.conceptProperties
                            )
                            cypher =
                                "MERGE (a:AbstractKB:\$abstractKBLabels {name: \$abstractKBName, properties: " +
                                        "\$abstractKBProperties)-[:KB_OF]->(c:Concept:\$sentenceLabels {name: " +
        ...
        fun createLogicalKB(
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val logicKBParameters = mapOf(
                        "logicKBName" to logicKBNode.logicKBName,
                        "logicKBLabels" to logicKBNode.logicKBLabels.joinToString(":"),
                        "logicKBProperties" to logicKBNode.logicKBProperties
                    )
                    val parameters = domainParameters + subdomainParameters + logicKBParameters
                    var cypher =
                        "MERGE (d:Domain:\$domainLabels {name: \$domainName, properties: \$domainProperties})" +
                                "-[:DOMAIN_OF]->(s:Subdomain:\$subdomainLabels {name: \$subdomainName, properties: " +
                                "\$subdomainProperties})-[:SUBDOMAIN_OF]->(a:logicKB:\$logicKBLabels {name: " +
                                "\$logicKBName, properties: \$logicKBProperties}) " +
                                "ON CREATE RETURN a ON MATCH RETURN null"
                    var result = session!!.run(cypher, parameters)
                    if (result.hasNext()) {
                        createdKBNode = result.next().get("a").asNode()
                        for (node in logicKBNode.logicNodes) {
                            val childParameters = mapOf(
                                "sentence" to node.logicSentence,
                                "sentenceLabels" to node.logicLabels.joinToString(":"),
                                "sentenceProperties" to node.logicProperties
                            )
                            cypher =
                                "MERGE (a:logicKB:\$logicKBLabels {name: \$logicKBName, properties: " +
                                        "\$logicKBProperties)-[:KB_OF]->(c:Concept:\$sentenceLabels {name: " +
                                        "\$sentence, properties: \$sentenceProperties}) " +
                                        "ON CREATE RETURN c ON MATCH RETURN null"
                            result = session!!.run(cypher, childParameters)
                            if (result.hasNext() && result != null) {
        ...
        fun createAbstractNode(
            ...
            subdomainNode: SubdomainGroupNode,
            abstractKBNode: AbstractKBNode,
            abstractNode: AbstractKnowledgeNode
        ): org.neo4j.driver.types.Node? {
            if (!checkForSubdomain(domainNode, subdomainNode)) {
                return null
            }
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val abstractNodeParameters = mapOf(
                        "abstractNodeName" to abstractNode.concept,
                        "abstractNodeLabels" to abstractNode.conceptLabels.joinToString(":"),
                        "abstractNodeProperties" to abstractNode.conceptProperties
                    )
                    val parameters =
                        domainParameters + subdomainParameters + abstractKBParameters + abstractNodeParameters
                    val cypher =
                        "MERGE (d:Domain:\$domainLabels {name: \$domainName, properties: \$domainProperties})" +
                                "-[:DOMAIN_OF]->(s:Subdomain:\$subdomainLabels {name: \$subdomainName, properties: " +
                                "\$subdomainProperties})-[:SUBDOMAIN_OF]->(a:AbstractKB:\$abstractKBLabels {name: " +
                                "\$abstractKBName, properties: \$abstractKBProperties})-[:KB_OF]->(c:Concept: " +
                                "\$abstractNodeLabels {name: \$abstractNodeName, properties: " +
                                "\$abstractNodeProperties}) ON CREATE RETURN c ON MATCH RETURN null"
                    val result = session!!.run(cypher, parameters)
                    if (result.hasNext()) {
        ...
        fun createLogicNode(
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val logicNodeParameters = mapOf(
                        "logicNodeName" to logicNode.logicSentence,
                        "logicNodeLabels" to logicNode.logicLabels.joinToString(":"),
                        "logicNodeProperties" to logicNode.logicProperties
                    )
                    ...
                    val cypher =
                        "MERGE (d:Domain:\$domainLabels {name: \$domainName, properties: \$domainProperties})" +
                                "-[:DOMAIN_OF]->(s:Subdomain:\$subdomainLabels {name: \$subdomainName, properties: " +
                                "\$subdomainProperties})-[:SUBDOMAIN_OF]->(a:logicKB:\$logicKBLabels {name: " +
                                "\$logicKBName, properties: \$logicKBProperties})-[:KB_OF]->(c:Concept: " +
                                "\$logicNodeLabels {name: \$logicNodeName, properties: \$logicNodeProperties}) " +
                                "ON CREATE RETURN c ON MATCH RETURN null"
                    val result = session!!.run(cypher, parameters)
                    if (result.hasNext()) {
                        createdNode = result.next().get("c").asNode()
            ...
            try {
                openSession('w').use { _ ->
                    ...
                    val fromNodeParameters: Map<String, Any> = if (fromNode is LogicNode) {
                        mapOf(
                            "fromLogicNodeName" to fromNode.logicSentence,
                            "fromLogicNodeLabels" to fromNode.logicLabels.joinToString(":"),
                            "fromLogicNodeProperties" to fromNode.logicProperties
                        ) as Map<String, LogicNode>
                    } else {
                        val fromAbstractNode = fromNode as AbstractKnowledgeNode
                        mapOf(
                            "fromAbstractNodeName" to fromAbstractNode.concept,
                            "fromAbstractNodeLabels" to fromAbstractNode.conceptLabels.joinToString(":"),
                            "fromAbstractNodeProperties" to fromAbstractNode.conceptProperties
                        ) as Map<String, AbstractKnowledgeNode>
                    }
                    val toNodeParameters: Map<String, Any> = if (toNode is LogicNode) {
                        mapOf(
                            "toLogicNodeName" to toNode.logicSentence,
                            "toLogicNodeLabels" to toNode.logicLabels.joinToString(":"),
                            "toLogicNodeProperties" to toNode.logicProperties
                        ) as Map<String, LogicNode>
                    } else {
                        val toAbstractNode = toNode as AbstractKnowledgeNode
                        mapOf(
                            "toAbstractNodeName" to toAbstractNode.concept,
                            "toAbstractNodeLabels" to toAbstractNode.conceptLabels.joinToString(":"),
                            "toAbstractNodeProperties" to toAbstractNode.conceptProperties
                        ) as Map<String, AbstractKnowledgeNode>
                    }
                    val parameters = relationshipParameters + fromNodeParameters + toNodeParameters
                    val cypher: String = when {
                        fromNodeParameters["fromLogicNodeName"] != null && toNodeParameters["toLogicNodeName"] !=
                                null -> {
                            "MERGE (from:LogicNode:\${parameters[\"fromLogicNodeLabels\"]} {name: " +
                                    "\${parameters[\"fromLogicNodeSentence\"]}, properties: " +
                                    "\${parameters[\"fromLogicNodeProperties\"]}})-" +
                                    "[r:\${parameters[\"relationshipName\"]} {properties: \$relationshipProperties}]" +
                                    "->(to:LogicNode:\${parameters[\"toLogicNodeLabels\"]} {name: " +
                                    "\${parameters[\"toLogicNodeName\"]}, properties: " +
                                    "\${parameters[\"toLogicNodeProperties\"]}}) ON CREATE RETURN r ON MATCH RETURN"
                        }

                        fromNodeParameters["fromLogicNodeName"] != null && toNodeParameters["toAbstractNodeName"] !=
                                null -> {
                            "MERGE (from:LogicNode:\${parameters[\"fromLogicNodeLabels\"]} {name: " +
                                    "\${parameters[\"fromLogicNodeName\"]}, properties: " +
                                    "\${parameters[\"fromLogicNodeProperties\"]}})" +
                                    "-[r:\${parameters[\"relationshipName\"]} {properties: \$relationshipProperties}]" +
                                    "->(to:AbstractNode:\${toNodeParameters[\"toAbstractNodeLabels\"]} " +
                                    "{name: \${toNodeParameters[\"toAbstractNodeName\"]}, properties: " +
                                    "\${toNodeParameters[\"toAbstractNodeProperties\"]}}) " +
                                    "ON CREATE RETURN r ON MATCH RETURN null"
                        }

                        fromNodeParameters["fromAbstractNodeName"] != null && toNodeParameters["toLogicNodeName"] !=
                                null -> {
                            "MERGE (from:AbstractNode:\${fromNodeParameters[\"abstractNodeLabels\"]} {name: " +
                                    "\${fromNodeParameters[\"abstractNodeName\"]}, properties: " +
                                    "\${fromNodeParameters[\"fromAbstractNodeProperties\"]}})" +
                                    "-[r:\${parameters[\"relationshipName\"]} {properties: \$relationshipProperties}]" +
                                    "->(to:LogicNode:\${toNodeParameters[\"toLogicNodeLabels\"]} {name: " +
                                    "\${toNodeParameters[\"toLogicNodeName\"]}, properties: " +
                                    "\${toNodeParameters[\"toLogicNodeProperties\"]}}) " +
                                    "ON CREATE RETURN r ON MATCH RETURN null"
                        }

                        fromNodeParameters["fromAbstractNodeName"] != null && toNodeParameters["toAbstractNodeName"] !=
                                null -> {
                            "MERGE (from:AbstractNode:\${fromNodeParameters[\"fromAbstractNodeLabels\"]} {name: " +
                                    "\${fromNodeParameters[\"fromAbstractNodeName\"]}, properties: " +
                                    "\${fromNodeParameters[\"fromAbstractNodeProperties\"]}})" +
                                    "-[r:\${parameters[\"relationshipName\"]} {properties: \$relationshipProperties}]" +
                                    "->(to:AbstractNode:\${toNodeParameters[\"toAbstractNodeLabels\"]} {name: " +
                                    "\${toNodeParameters[\"toAbstractNodeName\"]}, properties: " +
                                    "\${toNodeParameters[\"toAbstractNodeProperties\"]}}) " +
                                    "ON CREATE RETURN r ON MATCH RETURN null"
                        }
